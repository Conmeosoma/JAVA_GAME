...
public class Player extends Entity {
    ...
    public void update() // Runs 60 times every seconds.
    {
        ...
        } else if (attacking == true) // Need to adjust attack to "up" and "left" cuz player goes back a tile. Need to
                                      // adjust drawing start point(x,y)
        {
            attacking();
            ...
            else if (keyH.leftPressed == true) // For example when "keyH.upPressed == true", the else if blocks are not
                                               // working. And you cannot go anyway when you press up.
            {
                direction = "left";
        ...
        } else // This is for: If you release the key when you walking, change sprite num to 1
               // and use player's not-walking sprite.
        {
            standCounter++;
            if (standCounter == 20) // After you release the key player stands 20 frames last position then
                                    // spriteNum will be 1(default)
            {
                spriteNum = 1;
        ...
        // This needs to be outside of key if statement! // If player receive damage
        // from monster, player's gonna be invincible for a second
        if (invincible == true) {
            invincibleCounter++;
    ...
    public void contactMonster(int i) // CollisionChecker Method Implement //checkPlayer() : Checks who touches to
                                      // player //checkEntity() : Checks if player touches to an entity;
    {
        if (i != 999) {
    ...
    public void checkLevelUp() {
        while (exp >= nextLevelExp) {
            level++;
            exp = exp - nextLevelExp; // Example: Your exp is 4 and nextLevelExp is 5. You killed a monster and
                                      // receive 2exp. So, your exp is now 6. Your 1 extra xp will be recovered for
                                      // the next level.
            if (level <= 4) {
    ...
    public void draw(Graphics2D g2) {
        ...
        switch (direction) {
            case "up":
                ...
                {
                    tempScreenY = screenY - gp.tileSize; // Adjusted the player's position one tile to up. Explained why
                                                         // I did it at where I call attacking() in update().
                    if (spriteNum == 1) {
                        image = attackUp1;
                    }
                    if (spriteNum == 2) {
            ...
            case "left":
                ...
                if (attacking == true) // Attacking sprites
                {
                    tempScreenX = screenX - gp.tileSize; // Adjusted the player's position one tile left. Explained why
                                                         // I did it at where I call attacking() in update().
                    if (spriteNum == 1) {
                        image = attackLeft1;
                    }
        ...
        if (drawing == true) // for boss cutscene making player invisible to move camera.(Cuz camera movement
                             // based on player). Only draw the PlayerDummy
        {
         ...
         * solidArea.height); //PLAYER COLLISION CHECKER.RED RECTANGLE.
         * 
